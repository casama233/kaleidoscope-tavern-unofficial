...
def main():
 ...
 for lc in ['zh_TW','zh_CN','en_US']:
  ...
  for b in bindings:
   if b['kind'] in ['sofa','table','bar_counter','glassware_holder','holder','tilted_rack','circular_rack']:continue
 ...
 for page in pages:
  if page['id']==N+':c6_furniture':
   page['title']={'zh_TW':'酒館家具、酒架與掛畫','zh_CN':'酒馆家具、酒架与挂画','en_US':'Tavern furniture, racks and paintings'}
   page['body']={'zh_TW':'家具線已包含高腳凳、沙發、桌子、吧台、彩燈、4槽Glassware Holder、單瓶Holder、3款雙格吊燈、3槽Tilted Rack、Batch 13六槽Circular Rack，以及Batch 14全部14款掛畫。掛畫可依Java規則附著牆面、地面或天花板，保留四方向與1/16格薄碰撞；waterlogging暫不移植。Circular Rack的Java blocklist為空，因此除了本移植明示排除的Molotov，empty_bottle與24種品質飲品base均可存；world DP保存6個獨立精確*_q1..q6 ID。點擊按Java getLocalX/getLocalZ算圓周角度並分成6個60°槽位。每個有內容槽最多1個無碰撞helper，復用25種來源瓶型；來源renderer的6個圓周位置、scale0.82與各自yRot已適配。方塊亮度14、2/16薄底座碰撞及輸出2個的End Rod配方已還原。有瓶時每20tick以穩定spawnParticle在邊線生成minecraft:endrod，屬於Bedrock低頻cadence適配；Java紅石彈射仍不做。實機瓶型朝向、粒子密度、多人與觸控仍待驗收。','zh_CN':'家具线已包含4槽杯架、单瓶Holder、3槽Tilted Rack、Batch 13六槽Circular Rack等家具，并加入Batch 14全部14款挂画。挂画按Java规则支持墙面、地面或天花板附着，保留四方向与1/16格薄碰撞；waterlogging暂不移植。Circular Rack来源blocklist为空，因此除明确排除的Molotov外，empty_bottle与24种品质饮品base均可存，world DP保存6个独立精确*_q1..q6 ID。点击按Java圆周角度分6槽，每槽最多1个无碰撞visual helper。来源亮度14、2/16薄底座、输出2个的End Rod配方已还原；有瓶时用稳定spawnParticle低频生成endrod粒子。红石弹射仍不做。实机多人/触控仍待验收。','en_US':'The furniture line now includes the four-slot Glassware Holder, single Holder, three-slot Tilted Rack and Batch 13 six-slot Circular Rack alongside the other Tavern furniture, plus all 14 Batch 14 paintings. Paintings preserve Java wall/floor/ceiling attachment, four-way orientation and exact 1/16-thick collision; waterlogging remains not adapted. Circular Rack has an empty Java blocklist, so every shared bottle base is accepted except the port-excluded Molotov: empty_bottle plus all 24 quality-drink bases, with six independent exact *_q1..q6 IDs stored in world DP. Java facing-aware local X/Z angle selection is preserved as six 60-degree sectors. Each occupied slot has at most one collisionless helper reusing the 25 source bottle shapes; the six source positions, scale .82 and per-slot yaw are adapted. Source light 14, the 2/16 thin base collision and End Rod recipe output 2 are preserved. While occupied, a stable spawnParticle adapter emits minecraft:endrod on the rim every 20 ticks; this is a lower-frequency Bedrock cadence than Java client animateTick. Redstone projectile ejection remains NOT_ADAPTED; real-client orientation, particle density, multiplayer and touch remain NOT_RUN.'}
 ...
 dump(R/'docs/C6-FURNITURE-BINDINGS.json',{'bindings':bindings,'derived_icons':icon_records,'stools':16,'lights':17,'sofas':16,'tables':1,'bar_counters':1,'glassware_holders':1,'holders':1,'pendant_lamps':3,'tilted_racks':1,'circular_racks':1,'paintings':14,'engine_accepted':False})
 ...
 print('C6 generated: stools, sofas, connected table/counter, Holders + Pendant Lamps + Tilted/Circular Racks + 14 paintings and 17 light designs; official post-1.2 visual syncs retained (magenta light + gold grape texture + first c70eec cocktail cutout group).')
