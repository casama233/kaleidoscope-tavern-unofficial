...
def main():
 ...
 # Java creative tab exposes one max-level stack per base drink; lower qualities are internal runtime variants.
 quality=[]
 for ident,x in item_defs.items():
  m=re.search(r'_q([1-6]) check('no_native_experimental_block_container',all('minecraft:block_entity'not in d['components'] for d in block_defs.values()))
...
,ident)
  if not m:continue
  q=int(m.group(1));visible='menu_category'in x['description'];quality.append((ident,q,visible))
 check('quality_variant_count',len(quality)==144,{'actual':len(quality)})
 check('quality_creative_only_q6',all(visible==(q==6) for ident,q,visible in quality),[ident for ident,q,visible in quality if visible!=(q==6)])
 check('quality_creative_representatives',sum(visible for ident,q,visible in quality)==24)
 # Blocks exposed to Creative need resolvable tile localization in every advertised locale.
 for ident,x in block_defs.items():
  if 'menu_category' not in x['description']:continue
  key='tile.'+ident+'.name'
  check('block_display_name_localized:'+ident,all(key in lang_maps[lc] for lc in advertised),{'key':key,'missing':[lc for lc in advertised if key not in lang_maps[lc]]})
