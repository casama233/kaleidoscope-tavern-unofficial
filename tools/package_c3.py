#!/usr/bin/env python3
"""Package explicitly labeled C3 DEVELOPMENT packs. Never bundles Cookery or executable JARs."""
...
def main():
 ...
 if args.production:raise SystemExit('Refused: engine, gameplay and visual acceptance are not complete. C3 is development-only.')
 ...
 tests=json.loads((ROOT/'docs/TEST-RESULTS.json').read_text());assert tests['combined_fail']==0 and tests['combined_pass']>=195,'Run tests before packaging'
 assert json.loads((ROOT/'docs/REBUILD-REGRESSION.json').read_text())['pass'],'Run rebuild audit'
 src=out/'Tavern-Bedrock-C3.zip';dev=out/'Tavern-C3-Gameplay-DEV.mcaddon';demo=out/'Tavern-C3-Extension-Demo-OPTIONAL.mcpack'
 with zipfile.ZipFile(dev,'w')as z:
  add(z,'Tavern_C3_BP.mcpack',pack(ROOT/'runtime/BP'));add(z,'Tavern_C3_RP.mcpack',pack(ROOT/'runtime/RP'))
 demo.write_bytes(pack(ROOT/'examples/Tavern-Extension-Demo/BP'))
 mixdemo=out/'Tavern-C3-Mixology-Demo-OPTIONAL.mcpack';mixdemo.write_bytes(pack(ROOT/'examples/Tavern-Mixology-Demo/BP'))
 with zipfile.ZipFile(src,'w')as z:
  for f in sorted(ROOT.rglob('*')):
   if permitted(f,ROOT):add(z,'Tavern-Bedrock-C3/'+f.relative_to(ROOT).as_posix(),f.read_bytes())
 checks=[]
 ...
 result={'phase':'C3_CODE_DEVELOPMENT','cookery_bundled':False,'A17_art_source_included':True,'font_files_included':False,'engine_acceptance':'NOT_RUN','is_production_release':False,'artifacts':checks}
 (out/'Tavern-C3-DELIVERY-CHECK.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n')
 (out/'Tavern-C3-artifacts.sha256').write_text(''.join(f"{r['sha256']}  {r['file']}\n"for r in checks))
