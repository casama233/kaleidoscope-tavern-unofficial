...
export function circularRackVisualPose(slot,facing){check(Number.isInteger(slot)&&slot>=0&&slot<6,'CIRCULAR_RACK_SLOT');check(Number.isInteger(facing)&&facing>=0&&facing<=3,'INVALID_FACING');const b=BASE[slot],p=rotate(b.x,b.z,facing),fy=[0,90,180,-90][facing];return {offset:{x:p.x,y:.125,z:p.z},rotation:{x:0,y:wrapYaw(b.y+fy)}};}
