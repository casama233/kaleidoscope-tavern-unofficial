...
export const FACING=NS+':facing',CONNECTION=NS+':connection',AXIS=NS+':axis',POSITION=NS+':position',SEAT_ANCHOR=NS+':seat_anchor';
...
export function furnitureItem(id){if(typeof id!=='string'||!id.startsWith(NS+':'))return undefined;const short=id.slice(NS.length+1);for(const color of COLORS){if(short===color+'_bar_stool')return {kind:'stool',color};if(short===color+'_sofa')return {kind:'sofa',color};}for(const color of LIGHT_COLORS)if(short==='string_lights_'+color)return {kind:'light',color};if(['table','bar_counter','glassware_holder'].includes(short))return {kind:short};}
export function furnitureBlock(id){if(typeof id!=='string'||!id.startsWith(NS+':'))return undefined;const short=id.slice(NS.length+1);for(const color of COLORS){if(short==='stool_'+color)return {kind:'stool',color};if(short===color+'_sofa')return {kind:'sofa',color};}for(const color of LIGHT_COLORS)if(short==='light_'+color)return {kind:'light',color};if(['table','bar_counter','glassware_holder'].includes(short))return {kind:short};}
export function itemId(f){const valid=f&&((['stool','sofa'].includes(f.kind)&&COLORS.includes(f.color))||(f.kind==='light'&&LIGHT_COLORS.includes(f.color))||['table','bar_counter','glassware_holder'].includes(f.kind));check(valid,'UNKNOWN_FURNITURE');if(['table','bar_counter','glassware_holder'].includes(f.kind))return NS+':'+f.kind;return NS+':'+(f.kind==='stool'?f.color+'_bar_stool':f.kind==='sofa'?f.color+'_sofa':'string_lights_'+f.color);}
export function blockId(f){itemId(f);if(['sofa','table','bar_counter','glassware_holder'].includes(f.kind))return itemId(f);return NS+':'+(f.kind==='stool'?'stool_':'light_')+f.color;}
