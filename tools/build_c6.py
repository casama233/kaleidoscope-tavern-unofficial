...
def main():
 ...
 def recipe(short,record=True):
  ...
  tag_items={'c:ingots/iron':'minecraft:iron_ingot','c:nuggets/gold':'minecraft:gold_nugget'}
  for letter,val in d['key'].items():
   if 'item'in val:key[letter]={'item':val['item']}
   elif val.get('tag') in tag_items:key[letter]={'item':tag_items[val['tag']]}
   else:
 ...
 # Table: source has four positions and locks a connected run to X/Z. Rotated source models are already converted assets.
 table_geo={name:load(RP/f'models/entity/table_{name}.geo.json')['minecraft:geometry'][0]['description']['identifier'] for name in ['single','left','middle','right','left_rot','middle_rot','right_rot']}
 table_itemgeo=load(RP/'models/entity/item_display_table.geo.json')['minecraft:geometry'][0]['description']['identifier'];recipe('table')
 c=components({'identifier':table_geo['single']},{'*':{'texture':'kt_assets_a12_block_table','render_method':'alpha_test'}})
 ...
 table_perm=[{'condition':f"q.block_state('{N}:axis') == 0 && q.block_state('{N}:position') == {pos}",'components':{'minecraft:geometry':{'identifier':table_geo[name]}}}for pos,name in [(1,'right'),(2,'middle'),(3,'left')]]+[{'condition':f"q.block_state('{N}:axis') == 1 && q.block_state('{N}:position') == {pos}",'components':{'minecraft:geometry':{'identifier':table_geo[name]}}}for pos,name in [(1,'right_rot'),(2,'middle_rot'),(3,'left_rot')]]
 dump(BP/'blocks/table.json',{'format_version':'1.26.50','minecraft:block':{'description':{'identifier':N+':table','menu_category':{'category':'construction'},'states':{N+':axis':[0,1],N+':position':[0,1,2,3]}},'components':c,'permutations':table_perm}})
 bindings.append({'kind':'table','item':N+':table','block':N+':table','geometry_by_state':table_geo,'world_texture':'kt_assets_a12_block_table','item_geometry':table_itemgeo,'item_texture':'kt_assets_a17_item_display_table','positions':4,'axes':2,'waterlogged':False,'source_collision':'top_3_16'})
 # Bar Counter uses the exact same Java IConnectionBlock contract as SofaBlock.
 bar_geo={name:load(RP/f'models/entity/bar_counter_{name}.geo.json')['minecraft:geometry'][0]['description']['identifier'] for name in connection_names}
 bar_itemgeo=load(RP/'models/entity/item_display_bar_counter.geo.json')['minecraft:geometry'][0]['description']['identifier'];recipe('bar_counter')
 c=components({'identifier':bar_geo['single']},{'*':{'texture':'kt_assets_a10_block_deco_bar_counter','render_method':'alpha_test'}})
 ...
 perms=[{'condition':f"q.block_state('{N}:connection') == {i}",'components':{'minecraft:geometry':{'identifier':bar_geo[name]}}}for i,name in enumerate(connection_names)]+[{'condition':f"q.block_state('{N}:facing') == {i}",'components':{'minecraft:transformation':{'rotation':[0,-90*i,0]}}}for i in range(4)]
 dump(BP/'blocks/bar_counter.json',{'format_version':'1.26.50','minecraft:block':{'description':{'identifier':N+':bar_counter','menu_category':{'category':'construction'},'states':{N+':facing':[0,1,2,3],N+':connection':[0,1,2,3,4,5]}},'components':c,'permutations':perms}})
 bindings.append({'kind':'bar_counter','item':N+':bar_counter','block':N+':bar_counter','geometry_by_connection':bar_geo,'world_texture':'kt_assets_a10_block_deco_bar_counter','item_geometry':bar_itemgeo,'item_texture':'kt_assets_a17_item_display_bar_counter','connection_states':6,'waterlogged':False})
 ...
 for page in pages:
  if page['id']==N+':c6_furniture':
   page['title']={'zh_TW':'高腳凳、沙發、桌與吧台','zh_CN':'高脚凳、沙发、桌与吧台','en_US':'Stools, sofas, tables and bar counters'}
   page['body']={'zh_TW':'16色高腳凳、16色沙發與17款彩燈皆可合成、放置及回收。沙發沿用原作6種連接狀態，包含直線左右／中段與兩種轉角；不同顏色也能相連。每格沙發可空手坐1人，潛行離座，空位不保留helper實體；候選坐點0.45格來自原作0.5125座位錨點沿用現有-0.0625 Bedrock校正。高腳凳仍為0.8125格座點。彩燈可用染料換款。沙發waterlogging、背靠複合碰撞、Steve/Alex/手機多人座高仍待實機驗收。','zh_CN':'16色高脚凳、16色沙发与17款彩灯均可合成、放置和回收。沙发保留原作6种连接状态，且不同颜色可以连接；每格可空手坐1人，潜行离座，空位不会保留helper实体。waterlogging、复合碰撞与真实客户端座高仍待实机。','en_US':'All 16 stool colors, 16 sofa colors and 17 string-light designs are craftable/placeable/recoverable. Sofas preserve the six source connection states, including both corners, and different colors connect. Each sofa block seats one player; the invisible helper exists only while occupied. Candidate seat height .45 derives from the source .5125 anchor using the existing -.0625 Bedrock correction. Sofa waterlogging, compound backrest collision and real-client/mobile seating remain unverified.'}
 ...
 print('C6 generated: stools, sofas, lights, connected table and bar counter; official post-1.2 visual syncs retained (magenta light + gold grape texture + first c70eec cocktail cutout group).')
