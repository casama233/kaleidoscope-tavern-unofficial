...
function declaredComponents(kind){
 const dir=new URL('../runtime/BP/'+kind+'/',import.meta.url),ids=new Set();
 for(const file of fs.readdirSync(dir).filter(x=>x.endsWith('.json'))){const resource=JSON.parse(fs.readFileSync(new URL(file,dir))),def=resource[kind==='blocks'?'minecraft:block':'minecraft:item'];if(!def)continue;for(const components of [def.components,...(def.permutations??[]).map(p=>p.components)])for(const id of Object.keys(components??{}))if(id.startsWith(NS+':'))ids.add(id);}
 return [...ids].sort();
}
...
function waterTap({sourceLevel=1,destination='bottle',destinationLevel=1}={}){const at=target(),tap=dim.getBlock(at);tap.setPermutation(BlockPermutation.resolve(NS+':tap',{'minecraft:block_face':'east','minecraft:cardinal_direction':'east'}));const source=dim.getBlock({x:at.x-1,y:at.y,z:at.z});source.setPermutation(BlockPermutation.resolve('minecraft:cauldron',{cauldron_liquid:'water',fill_level:sourceLevel}));const below=dim.getBlock({x:at.x,y:at.y-1,z:at.z});if(destination==='bottle')below.setPermutation(BlockPermutation.resolve(NS+':bottle_empty',{'minecraft:cardinal_direction':'west'}));else below.setPermutation(BlockPermutation.resolve('minecraft:cauldron',{cauldron_liquid:'water',fill_level:destinationLevel}));return {tap,source,below};}
...
test('entrypoint registers one legacy-guide migrator and42 recipes',()=>{
 assert.equal(runtimeRegistry().allRecipes().length,42);assert.deepEqual([...regs.blocks.keys()].sort(),declaredComponents('blocks'));assert.deepEqual([...regs.items.keys()].sort(),declaredComponents('items'));
...
test('native Pressing Tub after-interact takes one ingredient once and respects foreign cancellation without a before event',()=>{
 const b=tub(),p=player();p.location={x:b.location.x+.5,y:b.location.y+.5,z:b.location.z+.5};hand(p,NS+':grape',4);operate(p,b,'use');hand(p,undefined);
 const component=regs.blocks.get(NS+':pressing_tub'),before=world.beforeEvents.playerInteractWithBlock,emit=before.emit.bind(before);let emitted=0;before.emit=(...args)=>{emitted++;return emit(...args);};
 try{component.onPlayerInteract({player:p,block:b});component.onPlayerInteract({player:p,block:b});system.advance();}finally{before.emit=emit;}
 assert.equal(emitted,0);assert.equal(state(b).slots[0].count,3);assert.equal(count(p,NS+':grape'),1);
 const c=tub(),q=player();q.location={x:c.location.x+.5,y:c.location.y+.5,z:c.location.z+.5};hand(q,'minecraft:stone',2);operate(q,c,'use');hand(q,undefined);component.onPlayerInteract({player:q,block:c,cancel:true});system.advance();
 assert.equal(state(c).slots[0].count,2);assert.equal(count(q,'minecraft:stone'),0);
});
...
test('tap custom interaction handles empty hand and deduplicates the before event',()=>{
 ...
 component.onPlayerInteract({player:p,block:tap});
...
test('empty water cauldron stays an empty open while lava cauldron fills a bottle into a Molotov',()=>{
 const p=player(),water=waterTap();water.source.setPermutation(BlockPermutation.resolve('minecraft:cauldron',{cauldron_liquid:'water',fill_level:0}));hand(p,undefined);click(p,water.tap);system.advance(5);assert.equal(water.tap.permutation.getState(TEST_ACCESS.TAP_OPEN),0);assert.equal(water.below.typeId,NS+':bottle_empty');
 const lava=waterTap();lava.source.setPermutation(BlockPermutation.resolve('minecraft:cauldron',{cauldron_liquid:'lava',fill_level:6}));click(p,lava.tap);system.advance(5);assert.equal(lava.tap.permutation.getState(TEST_ACCESS.TAP_OPEN),1);system.advance(25);assert.equal(lava.tap.permutation.getState(TEST_ACCESS.TAP_OPEN),0);assert.equal(lava.below.typeId,NS+':molotov');assert.equal(lava.source.permutation.getState('fill_level'),6);
});
...
test('filled barrel dismantles as Java; duplicate destruction drops only one',()=>{const b=barrel(player()),p=player();fill(p,b);dismantle(p,b);assert(barrelCells(b.location).every(c=>dim.getBlock(c).isAir));assert.equal(state(b),undefined);const empty=barrel(player()),q=player();const part=dim.getBlock({x:empty.location.x+1,y:empty.location.y,z:empty.location.z}),beforeDrops=dropCount(NS+':barrel');for(let i=0;i<2;i++){const e={player:q,block:part,cancel:false};world.beforeEvents.playerBreakBlock.emit(e);assert(e.cancel);}system.advance();assert.equal(count(q,NS+':barrel'),0);assert.equal(dropCount(NS+':barrel'),beforeDrops+1);assert(barrelCells(empty.location).every(c=>dim.getBlock(c).isAir));assert.equal(state(empty),undefined);});
test('visual synchronization reuses ingredient and liquid helpers and removes duplicates',()=>{
 const b=tub(),p=player();hand(p,NS+':grape',8);operate(p,b,'use');press(b,p,1);
 const key=machineKey(dim.id,b.location),find=()=>dim.getEntities().filter(e=>e.getDynamicProperty('kt:anchor')===key);
 assert.equal(find().length,2);
 const ingredients=find().find(e=>e.typeId===NS+':pressing_tub_ingredients_visual');
 const liquid=find().find(e=>e.typeId.includes('rig_liquid_'));
 assert(ingredients&&liquid);assert.equal(ingredients.properties[NS+':fill_stage'],7);
 syncVisuals(b,state(b));assert(find().includes(ingredients));assert(find().includes(liquid));
 const duplicate=dim.spawnEntity(liquid.typeId,liquid.location);
 for(const k of ['kt:anchor','kt:token','kt:core'])duplicate.setDynamicProperty(k,liquid.getDynamicProperty(k));
 syncVisuals(b,state(b));assert.equal(find().length,2);assert.equal(find().find(e=>e.typeId===liquid.typeId).properties['kt_art:amount'],125);
});
...
// Java 1.2.0 pressing parity: actual callbacks and rollback-sensitive failures.
test('a living mob presses, an item entity does not, and rejected contents eject once',()=>{
 const b=tub(),p=player();hand(p,NS+':grape',2);operate(p,b,'use');
 const mob=new Entity('minecraft:pig',dim,b.location);mob.health={currentValue:10};
 press(b,mob,.5);assert.equal(state(b).amount,125);assert.equal(state(b).slots[0].count,1);
 press(b,new Entity('minecraft:item',dim,b.location),1);assert.equal(state(b).slots[0].count,1);
 const wrong=tub();hand(p,'minecraft:stone',17);assert(click(p,wrong).cancel);const before=dropCount('minecraft:stone');
 press(wrong,mob,1);assert.equal(state(wrong).slots[0],null);assert.equal(dropCount('minecraft:stone')-before,17);
 press(wrong,mob,1);assert.equal(dropCount('minecraft:stone')-before,17);
});
test('rejected-ingredient save failure removes staged drops and preserves the complete tub slot',()=>{
 const b=tub(),p=player();hand(p,'minecraft:stone',17);operate(p,b,'use');const before=dropCount('minecraft:stone');
 world.failSet=true;press(b,p,1);assert.equal(state(b).slots[0].count,17);assert.equal(dropCount('minecraft:stone'),before);
});
test('breaking a full tub drops its remaining fruit once through the shared break route',()=>{
 const b=tub(),p=player();hand(p,NS+':grape',12);operate(p,b,'use');press(b,p,1);const before=dropCount(NS+':grape'),pots=dropCount(NS+':pressing_tub');
 for(let i=0;i<2;i++)world.beforeEvents.playerBreakBlock.emit({player:p,block:b,cancel:false});system.advance();
 assert.equal(b.typeId,'minecraft:air');assert.equal(state(b),undefined);assert.equal(dropCount(NS+':grape')-before,11);assert.equal(dropCount(NS+':pressing_tub')-pots,1);
});
test('barrel opening renders four independent ingredient counts, closes all contents and rebuilds after reopening',()=>{
 const p=player(),b=barrel(p);fill(p,b);const s=state(b);s.slots=[{id:'minecraft:apple',count:1},{id:'minecraft:sugar',count:2},{id:'minecraft:wheat',count:8},{id:'minecraft:ice',count:16}];
 syncVisuals(b,s);const key=machineKey(dim.id,b.location),find=()=>dim.getEntities().filter(e=>e.getDynamicProperty('kt:anchor')===key);
 const fruit=find().filter(e=>e.typeId.includes('barrel_ingredients_'));assert.equal(fruit.length,4);assert.deepEqual(fruit.map(e=>e.properties[NS+':count']),[1,2,5,9]);
 const pose=fruit.map(e=>({...e.properties}));syncVisuals(b,s);assert.deepEqual(fruit.map(e=>({...e.properties})),pose);
 syncVisuals(b,{...s,open:false});assert.equal(find().length,1);assert.equal(find()[0].typeId,NS+':barrel_closed_visual');
 syncVisuals(b,s);assert.equal(find().filter(e=>e.typeId.includes('barrel_ingredients_')).length,4);assert.equal(find().filter(e=>e.typeId.includes('rig_liquid_')).length,1);
});

test('fruit layout shows exact 1–64 counts, persists per-position scatter, and removes excess groups',()=>{
 const p=player(),b=tub(),s=state(b),key=machineKey(dim.id,b.location);
 const find=()=>dim.getEntities().filter(e=>e.typeId.includes('pressing_tub_ingredients')&&e.getDynamicProperty('kt:anchor')===key);
 for(const count of [1,2,16,17,32,33,48,49,64,9,1]){
  s.slots[0]={id:NS+':grape',count};syncVisuals(b,s);const all=find();assert.equal(all.length,Math.ceil(count/8));assert.equal(all.reduce((n,e)=>n+e.getProperty(NS+':fill_stage'),0),count);
  const poses=all.map(e=>({...e.properties}));syncVisuals(b,s);assert.deepEqual(find().map(e=>({...e.properties})),poses);
 }
 const other=tub();syncVisuals(other,s);const a=find()[0].getProperty(NS+':rot0'),c=dim.getEntities().find(e=>e.typeId.includes('pressing_tub_ingredients')&&e.getDynamicProperty('kt:anchor')===machineKey(dim.id,other.location));assert.notEqual(c.getProperty(NS+':rot0'),a);
});
test('successful pressing cancels fall damage in either event order without double consumption',()=>{
 const p=player(),b=tub();p.location={x:b.location.x+.5,y:b.location.y+.25,z:b.location.z+.5};hand(p,NS+':grape',3);operate(p,b,'use');
 const event=()=>({hurtEntity:p,damageSource:{cause:'fall'},damage:4,cancel:false});
 let e=event();world.beforeEvents.entityHurt.emit(e);assert(e.cancel);press(b,p,5);assert.equal(state(b).slots[0].count,3);system.advance();assert.equal(state(b).slots[0].count,2);
 system.advance(3);press(b,p,5);e=event();world.beforeEvents.entityHurt.emit(e);assert(e.cancel);assert.equal(state(b).slots[0].count,1);
 system.advance(3);const bad=tub();p.location={x:bad.location.x+.5,y:bad.location.y+.25,z:bad.location.z+.5};hand(p,'minecraft:stone');operate(p,bad,'use');e=event();world.beforeEvents.entityHurt.emit(e);assert.equal(e.cancel,false);
});
