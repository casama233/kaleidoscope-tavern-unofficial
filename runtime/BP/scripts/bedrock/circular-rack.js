...
function slotHelpers(block,slot){const a=circularRackAnchor(circularRackKey(block.dimension.id,block.location),slot);return block.dimension.getEntities({location:blockCenter(block.location),maxDistance:2}).filter(e=>e.getDynamicProperty(ANCHOR)===a);}
...
export function syncCircularRackVisuals(block,state=store.load(circularRackKey(block.dimension.id,block.location))){if(state)check(block?.typeId===CIRCULAR_RACK,'NOT_CIRCULAR_RACK');const facing=block?.permutation.getState(FACING)??0;for(let slot=0;slot<6;slot++){const item=state?.slots[slot]??null,all=slotHelpers(block,slot);if(!item){for(const e of all)discard(e,'orphans');continue;}const bottle=circularRackItem(item);check(bottle,'CIRCULAR_RACK_ITEM');all.sort((a,b)=>a.id.localeCompare(b.id));let e=all[0];if(e&&e.typeId!==externalVisual(HELPER,item)){discard(e,'repairs');e=undefined;}for(const x of all.slice(1))discard(x,'duplicates');const at=position(block,slot,facing),pose=circularRackVisualPose(slot,facing);if(!e){e=block.dimension.spawnEntity(externalVisual(HELPER,item),at,{initialRotation:pose.rotation.y});e.setDynamicProperty(ANCHOR,circularRackAnchor(circularRackKey(block.dimension.id,block.location),slot));e.addTag('kaleidoscope_tavern:visual_helper');circularRackDiagnostics.spawned++;}e.setProperty(NS+':storage_kind',bottle.kind);e.setRotation(pose.rotation);e.tryTeleport(at,{checkForBlocks:false});visuals.set(e.id,e);}}
...
export function installCircularRackEvents(){
 ...
 world.afterEvents.entityLoad.subscribe(e=>{if(isExternalVisual(e.entity.typeId,HELPER)){visuals.set(e.entity.id,e.entity);system.run(()=>maintainCircularRackVisual(e.entity));}});
