...
function dropped(id){return [...dim.entities.values()].filter(e=>!e.removed&&e.typeId==='minecraft:item'&&e.itemStack?.typeId===id).reduce((n,e)=>n+e.itemStack.amount,0);}
...
test('filled machine destruction is blocked; duplicate empty destruction drops only one',()=>{const b=barrel(player()),p=player();fill(p,b);code(()=>dismantle(p,b),'MACHINE_NOT_EMPTY');const empty=barrel(player()),q=player(),before=dropped(NS+':barrel'),soundCount=dim.sounds?.length??0;const part=dim.getBlock({x:empty.location.x+1,y:empty.location.y,z:empty.location.z});for(let i=0;i<2;i++){const e={player:q,block:part,cancel:false};world.beforeEvents.playerBreakBlock.emit(e);assert(e.cancel);}system.advance();assert.equal(count(q,NS+':barrel'),0);assert.equal(dropped(NS+':barrel'),before+1);assert.equal(dim.sounds?.slice(soundCount).filter(x=>x.id==='dig.wood').length,1);assert(barrelCells(empty.location).every(c=>dim.getBlock(c).isAir));assert.equal(state(empty),undefined);});
