...
function slotHelpers(block,slot){const a=cellarCabinetAnchor(cellarCabinetKey(block.dimension.id,block.location),slot);return block.dimension.getEntities({location:blockCenter(block.location),maxDistance:2}).filter(e=>e.getDynamicProperty(ANCHOR)===a);}
...
export function syncCellarCabinetVisuals(block,state=store.load(cellarCabinetKey(block.dimension.id,block.location))){if(state)check(block?.typeId===CELLAR_CABINET,'NOT_CELLAR_CABINET');const facing=block?.permutation.getState(FACING)??0;for(let slot=0;slot<9;slot++){const item=state?.slots[slot]??null,all=slotHelpers(block,slot);if(!item){for(const e of all)discard(e,'orphans');continue;}const bottle=cellarCabinetItem(item);check(bottle,'CELLAR_CABINET_ITEM');all.sort((a,b)=>a.id.localeCompare(b.id));let e=all[0];if(e&&e.typeId!==externalVisual(HELPER,item)){discard(e,'repairs');e=undefined;}for(const x of all.slice(1))discard(x,'duplicates');const pose=cellarCabinetVisualPose(slot,facing),at={x:block.location.x+pose.offset.x,y:block.location.y+pose.offset.y,z:block.location.z+pose.offset.z};if(!e){e=block.dimension.spawnEntity(externalVisual(HELPER,item),at,{initialRotation:pose.rotation.y});e.setDynamicProperty(ANCHOR,cellarCabinetAnchor(cellarCabinetKey(block.dimension.id,block.location),slot));e.addTag('kaleidoscope_tavern:visual_helper');cellarCabinetDiagnostics.spawned++;}e.setProperty(NS+':storage_kind',bottle.kind);e.setRotation({x:0,y:pose.rotation.y});e.tryTeleport(at,{checkForBlocks:false});visuals.set(e.id,e);}}
...
export function installCellarCabinetEvents(){
 ...
 world.afterEvents.entityLoad.subscribe(e=>{if(isExternalVisual(e.entity.typeId,HELPER)){visuals.set(e.entity.id,e.entity);system.run(()=>maintainCellarCabinetVisual(e.entity));}});
