...
system.run(()=>{
 try{
  ...
  registry=new ExtensionRegistry({recipes,pages:GUIDE_PAGES,fluids:FLUIDS,itemExists:id=>!!ItemTypes.get(id)});setRegistry(registry);installExtensionHost(registry);
  system.sendScriptEvent('kaleidoscope_cookery:api_ping','{}');
  console.warn('[Tavern C1] Independent books and extension v1 initialized. Development build: engine/visual acceptance required.');
 }catch(e){console.error('[Tavern C1] Startup halted: '+e);}
});
