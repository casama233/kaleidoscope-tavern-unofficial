...
def add(z,name,data):
 info=zipfile.ZipInfo(name,date_time=(2026,9,21,0,0,0));info.compress_type=zipfile.ZIP_DEFLATED;info.external_attr=(stat.S_IFREG|0o644)<<16;z.writestr(info,data)
def pack(folder):
 b=io.BytesIO()
...
def main():
 ...
 if args.production:raise SystemExit('Refused: engine, gameplay and visual acceptance are not complete. C7 is development-only.')
 ...
 tests=json.loads((ROOT/'docs/TEST-RESULTS.json').read_text());assert tests['combined_fail']==0 and tests['combined_pass']>=524,'Run tests before packaging'
 ...
 src=out/'Tavern-Bedrock-C7.zip';dev=out/'Tavern-C7-Gameplay-DEV.mcaddon';demo=out/'Tavern-C7-Extension-Demo-OPTIONAL.mcpack'
 ...
 mixdemo=out/'Tavern-C7-Mixology-Demo-OPTIONAL.mcpack';mixdemo.write_bytes(pack(ROOT/'examples/Tavern-Mixology-Demo/BP'))
 ...
 result={'phase':'C7_CODE_DEVELOPMENT','cookery_bundled':False,'A17_art_source_included':True,'font_files_included':False,'engine_acceptance':'NOT_RUN','is_production_release':False,'artifacts':checks}
