...
def main():
 ...
 for page in pages:
  if page['id']==N+':c5_effects':
   ...
   for lc in page['body']:page['body'][lc]+=('\nC6: Shriek Attack, Upside Down and Vision adapters are enabled. Six other types remain pending.'if lc=='en_US'else'\nC6更新：幽匿聲波、倒立與靈視適配已接入，其他6種效果仍待實作。')
   for lc in page['body']:page['body'][lc]=page['body'][lc].replace('其餘9種','其餘6種').replace('Other 9','Other 6')
 p.write_text('export const MIXOLOGY_PAGES = '+json.dumps(pages,ensure_ascii=False,indent=2)+';\n',encoding='utf-8')
 ...
 for effect in ['shriek_attack','upside_down','vision']:
 ...
 coverage['scope']='Players as effect owners; Shriek is explicitly PvE-only; Upside Down and Vision use bounded Bedrock entity queries plus exact AABB checks; other timed effects as C5'
