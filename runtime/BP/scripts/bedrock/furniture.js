...
/** New ordinary furniture uses native placement_direction; custom facing is legacy-only migration data. */
export function furnitureFacing(block){const legacy=block?.permutation?.getState(FACING)??0;if(Number.isInteger(legacy)&&legacy>0&&legacy<=3)return legacy;const cardinal=block?.permutation?.getState(NATIVE_FACING);return cardinal?facingFromCardinal(cardinal):0;}
export function migrateFurnitureFacing(block){const f=furnitureBlock(block?.typeId);if(!f||!NATIVE_PLACEMENT_KINDS.has(f.kind)||f.kind==='table')return false;const legacy=block.permutation.getState(FACING)??0;if(!Number.isInteger(legacy)||legacy<=0||legacy>3)return false;block.setPermutation(block.permutation.withState(NATIVE_FACING,cardinalForFacing(legacy)).withState(FACING,0));return true;}
...
export function registerFurnitureComponents({blockComponentRegistry:r}){r.registerCustomComponent(NS+':stool',{onPlace:e=>optional(()=>ensureSeat(e.block)),onTick:e=>optional(()=>{migrateFurnitureFacing(e.block);ensureSeat(e.block);})});r.registerCustomComponent(NS+':sofa',{onPlace:e=>optional(()=>syncSofaNeighborhood(e.block.dimension,e.block.location)),onTick:e=>optional(()=>{migrateFurnitureFacing(e.block);syncSofa(e.block);})});r.registerCustomComponent(NS+':table',{onPlace:e=>optional(()=>syncTableNeighborhood(e.block.dimension,e.block.location)),onTick:e=>optional(()=>syncTable(e.block))});r.registerCustomComponent(NS+':bar_counter',{onPlace:e=>optional(()=>syncBarCounterNeighborhood(e.block.dimension,e.block.location)),onTick:e=>optional(()=>{migrateFurnitureFacing(e.block);syncBarCounter(e.block);})});r.registerCustomComponent(NS+':pendant_lamp',{onTick:e=>optional(()=>repairVerticalDouble(e.block))});r.registerCustomComponent(NS+':string_light',{});}
export function installFurnitureEvents(){
 ...
 registerProtectedBreakRoute({
  ...
  capture:({player,block})=>({legacyFacing:block.permutation.getState(FACING),nativeFacing:block.permutation.getState(NATIVE_FACING),hand:handSnapshot(player)}),
  verify:({player,block,snapshot,dimension,location})=>{isNear(player,dimension,location);sameHand(player,snapshot.hand);check(block.permutation.getState(FACING)===snapshot.legacyFacing&&block.permutation.getState(NATIVE_FACING)===snapshot.nativeFacing,'BLOCK_CHANGED');},
