"""Offline A12 model/material inspection, NOT game screenshots or engine acceptance.
...
def main():
    ...
    for r in rows:
        ...
        if r.get('batch')=='A12' or not(P/f'{r["id"]}.png').exists():
    ...
    print('Preview data refreshed:',len(models),'appearances; newly rendered A12 sources only.')
...
def make_overview(rows):
    ...
    d.text((38,20),'TAVERN A12 / 家具・吊燈・素面告示牌',font=font(34),fill=(239,226,201))
    d.text((38,78),'17 份原作分件 + 5 份組合展示  |  6 張原作 PNG 路徑（4 份獨立圖像）',font=font(21),fill=(174,195,204))
    d.text((38,116),'離線幾何投影；不是 Minecraft 截圖。原尺寸保留，尚未引擎驗收。',font=font(19),fill=(174,195,204))
    lookup={r['id']:r for r in rows}
    names=['stepladder_assembled','sandwich_board_assembled','bell_pendant_lamp_assembled',
           'blue_pendant_lamp_assembled','yellow_pendant_lamp_assembled','table_single',
           'table_left','table_middle','table_right','table_left_rot','table_middle_rot','table_right_rot']
    ...
    d.text((38,1221),'組合展示為實體候選；沒有自動拼接、寫字、座位、照明或其他玩法邏輯。',font=font(20),fill=(213,201,171))
    sheet.save(P/'A12-overview.png')
