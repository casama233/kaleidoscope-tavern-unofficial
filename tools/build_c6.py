...
def main():
 ...
 for page in pages:
  ...
  if page['id']==N+':cocktail_effects/screwdriver':
   page['body']={lc:body.replace('[未實作]','[C6 Grumm 倒立適配已接入；名稱牌待實機]').replace('[not implemented]','[C6 Grumm adapter implemented; nameplate engine test pending]')for lc,body in page['body'].items()}
  if page['id']==N+':cocktail_effects/mojito':
   page['body']={lc:body.replace('[未實作]','[C6 靈視發光適配已接入；引擎驗收待執行]').replace('[not implemented]','[C6 Vision glowing adapter implemented; engine test pending]')for lc,body in page['body'].items()}
  if page['id']==N+':cocktail_effects/nether_special':
   page['body']={lc:body.replace('[未實作]','[C6 摸金校尉卸裝適配已接入；實機驗收待執行]').replace('[not implemented]','[C6 Tomb Raider disarm adapter implemented; engine test pending]')for lc,body in page['body'].items()}
 pages.extend([
 ...
 for effect in ['shriek_attack','upside_down','vision','tomb_raider']:
 ...
 coverage['scope']='Players as effect owners; Shriek is explicitly PvE-only; Upside Down/Vision use bounded queries; Tomb Raider maps Java entity tags and 40-tick pickup delay; other timed effects as C5'
 ...
 coverage['vision']={'source':'Java 1.2.0: every 50 ticks, radius=min(amplifier+1,3)*6; other living entities get Glowing for 60 ticks; sound only for newly glowing targets','adapter':'Timed custom status; 5-tick scheduler detects crossed 50-tick countdown boundaries, exact AABB overlap, native glowing 60 ticks and kt_assets_a17.effect.vision sound','divergence':'Pulse may occur within the existing 5-tick adapter cadence rather than the exact Java entity tick; client rendering/audio need engine acceptance','engine_tested':False}
 coverage['tomb_raider']={'source':'Java 1.2.0 LivingHurtEvent: Tomb Raider attacker; #minecraft:skeletons plus zombie/piglin/illager/witch targets; target RNG < 0.3F; mainhand durability to max-1; drop with pickup delay40','adapter':'Bedrock entityHurt; mapped target IDs including skeleton_horse/bogged and zombie_pigman; Java float32 chance; equippable Mainhand; durability max-1; spawnItem plus entityItemPickup cancel until tick+40','divergence':'C6 custom effect owners are players; Bedrock entityHurt is after damage, so lethal-hit ordering needs engine acceptance','engine_tested':False}
 dump(R/'docs/C6-EFFECT-COVERAGE.json',coverage)
 build=load(R/'docs/C5-BUILD.json');build.update({'phase':'C6','version':V,'native_crafting_recipes':43,'effect_hooks':'native + BloodyMary + XPDrain/Zenith/Shriek/UpsideDown/Vision/TombRaider adapters','custom_effect_types_pending':coverage['not_implemented'],'furniture':{'stools':16,'lights':17,'new_shaped_recipes':33,'source_anchor_y':.875,'source_explicit_rider_offset':-.0625,'native_seat_y':.8125,'light_emission':15},'custom_effects':dict(build['custom_effects'],shriek_attack='native sonicBoom/PvE-only ray adapter',upside_down='Grumm naming adapter over Java 16-block inflated AABB using Bedrock mob-family query',vision='native Glowing radius adapter on Java 50-tick countdown cadence',tomb_raider='30% mapped-mob mainhand disarm/drop adapter with 40-tick pickup lock'),'engine_acceptance':'NOT_RUN'})
