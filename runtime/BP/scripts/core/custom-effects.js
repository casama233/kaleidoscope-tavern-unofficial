...
export const CUSTOM_IMPLEMENTED=Object.freeze({
 ...
 'kaleidoscope_tavern:slightly_tipsy':'java_waveform_yaw_adapter'
});
...
const instant=id=>CUSTOM_INSTANT.includes(id)||externalEffectDefinition(id)?.mode==='instant';
const supported=id=>!!CUSTOM_IMPLEMENTED[id]||externalEffectDefinition(id)?.mode==='timed';
// An absent addon must not make a saved world unreadable. Unknown external rows
// remain structurally validated and inert; only registration allows application.
const stored=id=>supported(id)||typeof id==='string'&&/^[a-z][a-z0-9_]{1,47}:[a-z0-9_./-]+$/.test(id)&&!id.startsWith('kaleidoscope_tavern:');
...
export function readStatus(raw){if(raw===undefined)return {schema:1,entries:[]};const d=JSON.parse(raw);check(d?.schema===1&&Array.isArray(d.entries)&&d.entries.length<=32,'CUSTOM_EFFECT_SCHEMA');if(d.migrations!==undefined)check(d.migrations&&typeof d.migrations==='object'&&!Array.isArray(d.migrations)&&Object.keys(d.migrations).length<=64&&Object.entries(d.migrations).every(([k,v])=>/^[a-z][a-z0-9_]{1,47}$/.test(k)&&typeof v==='string'&&/^[0-9a-f]{8}$/.test(v)),'CUSTOM_MIGRATION_SCHEMA');if(d.legacyClosed!==undefined)check(typeof d.legacyClosed==='boolean','CUSTOM_MIGRATION_SCHEMA');for(const e of d.entries){check(stored(e.id)&&!instant(e.id),'CUSTOM_EFFECT_SCHEMA');integer(e.ticks,1,20000000);integer(e.amplifier,0,255);}return d;}
/** Retain stronger short effects and weaker longer ones; both durations elapse online. */
export function addStatus(state,id,ticks,amplifier){check(supported(id)&&!instant(id),'CUSTOM_EFFECT_UNSUPPORTED');integer(ticks,1,20000000);integer(amplifier,0,255);const s=clone(state);const old=s.entries.find(e=>e.id===id&&e.amplifier===amplifier);if(old)old.ticks=Math.max(old.ticks,ticks);else s.entries.push({id,ticks,amplifier});check(s.entries.length<=32,'CUSTOM_EFFECT_LIMIT');return s;}
export function removeStatus(state,id){const s=clone(state);s.entries=s.entries.filter(e=>e.id!==id);return s;}
export function advanceStatus(s,ticks){integer(ticks,0,2147483647);return {...s,schema:1,entries:s.entries.map(e=>({...e,ticks:e.ticks-ticks})).filter(e=>e.ticks>0)};}
