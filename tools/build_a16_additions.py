"""A16 acquired art: six remaining stool colors and four distinct string-light designs.
...
NS = 'kt_assets_a16'
NEW_STOOL_COLORS = {'lime':'淺綠','pink':'粉紅','gray':'灰','light_gray':'淺灰','purple':'紫','green':'綠'}
NEW_LIGHT_COLORS = {'colorless':'無','brown':'棕','cyan':'青','gray':'灰'}
...
def compose_color_stool(color: str):
    ...
    target = RP / f'textures/kt_derived/a16/{name}_atlas.png'
    ...
    for bone in model['bones']:
        ...
        for cube in bone['cubes']:
            for face in cube['faces'].values():
                face['uv'][0] += 64
                face['uv'][2] += 64
    model.update(name=name, texture=f'derived/a16/{name}_atlas', texture_size=[128, 32],
...
def add_assets():
    ...
    if any(r.get('batch') == 'A16' for r in reg['models']):
        raise ValueError('Rebuild A15 before appending A16')
    ...
    def output(model, parents, mapping, zh, en, category, extra,
        ...
        for loc in labels:
            ...
            labels[loc][('entity.' if entity else 'tile.') + ident + '.name'] = title + ' [A16]'
        row = {
            ...
            'engine_visual_test': 'NOT_RUN', 'batch': 'A16', 'category': category, 'title_zh': zh, 'title_en': en,
    ...
    for color, title in NEW_LIGHT_COLORS.items():
        ...
        output(model, parents, mapping, ('無色彩燈' if color=='colorless' else title+'色彩燈'), color+' String Lights','string_lights',
    ...
    dump(RP/'textures/terrain_texture.json',{'resource_pack_name':'tavern_a16','texture_name':'atlas.terrain','texture_data':terrain})
    dynamic['build']='A16'; dump(ROOT/'interfaces/dynamic-visuals.json',dynamic)
    dump(ROOT/'docs/A16-DERIVED-ATLASES.json',atlases)
    reg.update(batch='A16 cumulative',source_files_verified=len(lock['assets']))
    dump(ROOT/'asset-conversion.json',reg);dump(ROOT/'tools/render-data.json',canonical)
    f=BP/'functions/kt_a16/string_lights.mcfunction';f.parent.mkdir(parents=True,exist_ok=True)
    f.write_text('# Gives four art-only inspection objects; does not place blocks or clear the world.\n'+'\n'.join('give @s '+NS+':string_lights_'+c+' 1'for c in NEW_LIGHT_COLORS)+'\n')
    # Put summons in documentation, not a function that could spawn all entities accidentally.
    dump(ROOT/'docs/A16-STOOL-INSPECTION.json',{'commands':[f'/summon {NS}:bar_stool_{c} ~ ~ ~'for c in NEW_STOOL_COLORS],'kind':'entity-only','automatically_executed':False,'seating':False})
    src=[x for x in lock['assets']if x.get('batch')=='A16']
    dump(ROOT/'docs/A16-SOURCE-MANIFEST.json',{'commit':lock['commit'],'files':src,'official_release_jar_compared':False,'acquisition':'Pinned connector reads; source bytes accepted only after full Git blob SHA-1 equality. No generated replacement art.'})
    dump(ROOT/'docs/A16-DELTA.json',{'batch':'A16','new_source_files':len(src),'new_geometries':len(NEW_LIGHT_COLORS),'new_appearances':len(new),'new_editor_files':len(new),'new_original_png_paths':sum(x['path'].endswith('.png')for x in src),'new_derived_atlases':len(atlases),'bar_stool_colors':16,'string_light_designs':8,'total_geometries':len(list((RP/'models/entity').glob('*.geo.json'))),'total_appearances':sum(r['status']=='CONVERTED_CANDIDATE'for r in reg['models']),'total_editor_files':len(list((ROOT/'editor').glob('*.bbmodel'))),'total_original_png_paths':len(list((AS/'textures').rglob('*.png'))),'all_art_completed':False,'gameplay':'NONE','engine_test':'NOT_RUN'})
    return reg
