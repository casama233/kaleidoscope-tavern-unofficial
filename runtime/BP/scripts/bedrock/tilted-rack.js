...
export function syncTiltedRackVisuals(block,state=store.load(tiltedRackKey(block.dimension.id,block.location))){if(state)check(block?.typeId===TILTED_RACK,'NOT_TILTED_RACK');const facing=block?.permutation.getState(FACING)??0;for(let slot=0;slot<3;slot++){const item=state?.slots[slot]??null,all=slotHelpers(block,slot);if(!item){for(const e of all)discard(e,'orphans');continue;}const bottle=tiltedRackItem(item);check(bottle,'TILTED_RACK_ITEM');all.sort((a,b)=>a.id.localeCompare(b.id));let e=all[0];for(const x of all.slice(1))discard(x,'duplicates');const at=position(block,slot,facing),pose=tiltedRackVisualPose(slot,facing);if(!e){e=block.dimension.spawnEntity(HELPER,at);e.setDynamicProperty(ANCHOR,tiltedRackAnchor(tiltedRackKey(block.dimension.id,block.location),slot));e.addTag('kaleidoscope_tavern:visual_helper');tiltedRackDiagnostics.spawned++;}e.setProperty(NS+':storage_kind',bottle.kind);e.setRotation(pose.rotation);e.tryTeleport(at,{checkForBlocks:false});visuals.set(e.id,e);}}
