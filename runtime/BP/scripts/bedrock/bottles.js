...
export function placeEmptyBottle(player,target){
 ...
 const c=inventory(player),plan=planInventory(c,player.selectedSlotIndex,player.getGameMode()===GameMode.Creative?0:1,[],makeStack),old=b.permutation,facing=facingFor(player);
 return locks.with([emptyKey(b),player.id],()=>commitInventory(plan,c,()=>b.setPermutation(BlockPermutation.resolve(EMPTY_BLOCK,{[FACING]:facing})),()=>b.setPermutation(old)));
}
export function takeEmptyBottle(player,b){
 ...
 return locks.with([emptyKey(b),player.id],()=>commitInventory(plan,c,()=>b.setType('minecraft:air'),()=>b.setPermutation(old)));
}
...
export function installBottleEvents(){
 ...
 // Empty BottleBlockItem uses normal Java BlockItem placement after clicked-block use PASSes.
 ...
 registerProtectedBreakRoute({
  id:'bottles',
  isBlock:block=>DISPLAY_IDS.has(block?.typeId)||block?.typeId===EMPTY_BLOCK,
