...
export function placeEmptyBottle(player,target){
 ...
 return locks.with([k,player.id],()=>{
  ...
  const oldBlock=b.permutation,c=inventory(player),plan=planInventory(c,player.selectedSlotIndex,1,[],makeStack),facing=playerFacing(player);
  commitInventory(plan,c,()=>b.setPermutation(emptyPermutation(facing)),()=>b.setPermutation(oldBlock));return b;
 });
}
export function takeEmptyBottle(player,b){
 ...
 return locks.with([k,player.id],()=>{
  ...
  commitInventory(plan,c,()=>b.setPermutation(BlockPermutation.resolve('minecraft:air')),()=>b.setPermutation(old));return EMPTY_ITEM;
 });
}
