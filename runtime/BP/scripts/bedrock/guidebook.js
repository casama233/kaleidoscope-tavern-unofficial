...
export async function openGuide(player,registry,names,diagnostics,{recipesOnly=false}={}){
 ...
 try{while(true){
  ...
  if(state.view==='home'){
   ...
   button(tr('沉浸／計時輔助','Immersion / timing assistance'),async()=>{const key='kaleidoscope_tavern:timing_assist',enabled=player.getDynamicProperty(key)===true;const r=await new ModalFormData().title(tr('計時輔助','Timing assistance')).toggle(tr('顯示精確tick與結果窗口（預設關）','Show precise ticks and result windows (default off)'),{defaultValue:enabled}).dropdown(tr('手持輸入（原生未實機驗收）','Held input (native awaits engine validation)'),[tr('原生：按住／鬆手','Native hold / release'),tr('相容：兩次使用','Compatibility: two clicks')],{defaultValueIndex:player.getDynamicProperty('kaleidoscope_tavern:shaker_input_mode')==='toggle'?1:0}).show(player);if(!r.canceled){player.setDynamicProperty(key,r.formValues?.[0]===true);player.setDynamicProperty('kaleidoscope_tavern:shaker_input_mode',r.formValues?.[1]===1?'toggle':'native');}return state;});
