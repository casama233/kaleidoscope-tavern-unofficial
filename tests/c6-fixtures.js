...
Entity.prototype.getAABB=function(){if(this.failAABB)throw Error('AABB_FAIL');return this.box??{center:{x:this.location.x,y:this.location.y+1,z:this.location.z},extent:{x:.3,y:1,z:.3}};};\nPlayer.prototype.getAABB=function(){if(this.failAABB)throw Error('AABB_FAIL');return this.box??{center:{x:this.location.x,y:this.location.y+.9,z:this.location.z},extent:{x:.3,y:.9,z:.3}};};
...
Dimension.prototype.spawnEntity=function(id,pos){
 if(this.failSpawn)throw Error('SPAWN_FAIL');const e=spawn.call(this,id,pos);
 if(id.startsWith('kaleidoscope_tavern:seat_')){
