...
| bottled_drink | drink：wine/champagne/honey_wine/ice_wine/vodka，以及 A7 九類（見下）；count：1–4；另加入 glowflower_brew/luminous_bride | 64 |
...
| cocktail | drink：emerald/screwdriver/depth_charge/mojito/signature_cocktail/mystery_cocktail | 6 |
| shaker | 空 state | 1 |
...
原作 Brandy、Carignan、Sunset Glow 只有1–3瓶來源，但這三類尚未轉出，所以目前不在可查詢清單；不能靠來源範圍表宣稱接口已支持。這些 state 名稱是接口參數，**不會自動讓沙發連接、葡萄生長或桶蓋開啟**。例如 barrel 的兩個選項仍對應兩個靜態展示實體。
...
`build_assets.py` 先執行保留的 A4 資源建置，依序轉出 A6、A7、A8 原作資源，再補舊方塊的物品外觀元件、修正 bridge type、提高自己的 manifest 版本，最後重新套用已驗證格式的 Cookery 鎖定資料。其他外部依賴版本不會一起升成 Tavern 版本。
...
`interfaces/pose-candidates.json` 記錄獨立可選PoseLab，30份幾何／45外觀。主包未套用姿態，不把optional overlay算成runtime完成。特調tint槽位有對照但沒有任意RGB運行時接口。
