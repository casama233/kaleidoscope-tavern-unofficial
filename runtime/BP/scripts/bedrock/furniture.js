...
export function placeFurniture(player,target,{face='Up'}={}){
 ...
 locks.with([key,player.id],()=>{
  ...
  const states={};
  if(seated(f)){states[FACING]=facingForYaw(player.getRotation().y);states[TABLE_CARDINAL]=['north','east','south','west'][states[FACING]];if(f.kind==='sofa')states[CONNECTION]=SOFA_CONNECTION.SINGLE;}
