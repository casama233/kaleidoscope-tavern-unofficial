...
def main():
 ...
 for page in pages:
  if page['id']==N+':c5_effects':
   ...
   for lc in page['body']:page['body'][lc]+=('\nC6: Shriek Attack PvE, Upside Down, Vision, Tomb Raider and Ardent Heat adapters are now enabled. Four other types remain pending.'if lc=='en_US'else'\nC6更新：幽匿特調聲波、倒立、靈視、摸金校尉與醇熱適配已接入，其他4種效果仍待實作。')
   for lc in page['body']:page['body'][lc]=page['body'][lc].replace('其餘9種','其餘4種').replace('Other 9','Other 4')
 p.write_text('export const MIXOLOGY_PAGES = '+json.dumps(pages,ensure_ascii=False,indent=2)+';\n',encoding='utf-8')
 ...
 dump(R/'docs/C6-SOURCE-AUDIT.json',{'jar_sha256':'03f35e1e614953b22cd1f5e34345613f3a6a283bf1b1c99659b57d58970edeff','files':source_records,'jar_executed':False,'original_art_modified':False,'post_1_2_official_visual_sync':['c4ec1880bd44cf3139d3ba744ab30bb379cf1416:string_lights_magenta']})
 ...
 for effect in ['shriek_attack','upside_down','vision','tomb_raider','ardent_heat']:
 ...
 coverage['scope']='Players as custom-effect owners; Shriek/Upside Down/Vision/Tomb Raider/Ardent Heat use explicit Bedrock adapters; remaining timed effects as C5'
 ...
 build=load(R/'docs/C5-BUILD.json');build.update({'phase':'C6','version':V,'native_crafting_recipes':43,'effect_hooks':'native + BloodyMary + XPDrain/Zenith/Shriek/UpsideDown/Vision/TombRaider/ArdentHeat adapters','custom_effect_types_pending':coverage['not_implemented'],'furniture':{'stools':16,'lights':17,'new_shaped_recipes':33,'source_anchor_y':.875,'source_explicit_rider_offset':-.0625,'native_seat_y':.8125,'light_emission':15},'custom_effects':dict(build['custom_effects'],shriek_attack='native sonicBoom/PvE-only ray adapter',upside_down='Grumm naming adapter over Java 16-block inflated AABB using Bedrock mob-family query',vision='native Glowing radius adapter on Java 50-tick countdown cadence',tomb_raider='30% disarm/drop adapter with 40-tick pickup lock',ardent_heat='per-tick sprint 3x3 source-stone breaking adapter with exhaustion/armor/bare-collision costs'),'upstream_visual_sync':[{'commit':'c4ec1880bd44cf3139d3ba744ab30bb379cf1416','asset':'string_lights_magenta','fix':'12 rotated zero-thickness planes now have reverse faces; no texture change','engine_acceptance':'NOT_RUN'}],'engine_acceptance':'NOT_RUN'})
