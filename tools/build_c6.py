...
def main():
 ...
 def recipe_ingredient(val):
  ...
  assert tag in {'minecraft:planks','minecraft:fences'};return {'tag':tag}
 ...
 tc={'format_version':'1.10.0','minecraft:client_entity':{'description':{'identifier':N+':tilted_rack_bottle_visual','materials':{'default':'entity_alphatest'},'textures':tt,'geometry':tg,'scripts':{'scale':'0.9'},'render_controllers':['controller.render.kt_runtime.tilted_rack_bottle']}}}
 ...
 incense_specs={
  'sakura':{'small':'kt_assets_a17:sakura_incense','large':'kt_assets_a17:sakura_incense','source_large':'minecraft:cherry_leaves','large_adapter':'STYLE_PARTICLE_FALLBACK','y_offset':-2,'y_range':16},
  'pine':{'small':'kt_assets_a17:pine_incense','large':'kt_assets_a17:pine_incense_large','source_large':'kaleidoscope_tavern:pine_incense_large','large_adapter':'SOURCE_CUSTOM','y_offset':-2,'y_range':16},
  'ginkgo':{'small':'kt_assets_a17:ginkgo_incense','large':'kt_assets_a17:ginkgo_incense_large','source_large':'kaleidoscope_tavern:ginkgo_incense_large','large_adapter':'SOURCE_CUSTOM','y_offset':-2,'y_range':16},
  'spore':{'small':'kt_assets_a17:spore_incense','large':'kt_assets_a17:spore_incense','source_large':'minecraft:spore_blossom_air','large_adapter':'STYLE_PARTICLE_FALLBACK','y_offset':-2,'y_range':16},
  'catnip':{'small':'kt_assets_a17:catnip_incense','large':'kt_assets_a17:catnip_incense_large','source_large':'kaleidoscope_tavern:catnip_incense_large','large_adapter':'SOURCE_CUSTOM','y_offset':-2,'y_range':16},
  'snow':{'small':'kt_assets_a17:snow_incense','large':'kt_assets_a17:snow_incense_large','source_large':'kaleidoscope_tavern:snow_incense_large','large_adapter':'SOURCE_CUSTOM','y_offset':-2,'y_range':16},
  'butterfly':{'small':'kt_assets_a17:butterfly_incense','large':'kt_assets_a17:butterfly_incense_large','source_large':'kaleidoscope_tavern:butterfly_incense_large','large_adapter':'SOURCE_CUSTOM','y_offset':-2,'y_range':16},
  'firefly':{'small':'kt_assets_a17:firefly_incense','large':'kt_assets_a17:firefly_incense_large','source_large':'kaleidoscope_tavern:firefly_incense_large','large_adapter':'SOURCE_CUSTOM','y_offset':-.67,'y_range':5.33}
 }
 for style,speci in incense_specs.items():
  short=style+'_incense';recipe(short,False);closed=vis[short+'_closed'];opened=vis[short+'_open']
  ic=components(copy.deepcopy(closed['binding']['geometry']),copy.deepcopy(closed['binding']['materials']))
  ic.update({'minecraft:destructible_by_mining':{'seconds_to_destroy':0.0},'minecraft:destructible_by_explosion':{'explosion_resistance':0.0},'minecraft:collision_box':{'origin':[-3,0,-3],'size':[6,7,6]},'minecraft:selection_box':{'origin':[-3,0,-3],'size':[6,7,6]},'minecraft:tick':{'interval_range':[3,3],'looping':True},N+':incense':{},'minecraft:item_visual':copy.deepcopy(closed['binding']['item_visual'])})
  ip=[{'condition':f"q.block_state('{N}:open') == 1",'components':{'minecraft:geometry':copy.deepcopy(opened['binding']['geometry']),'minecraft:material_instances':copy.deepcopy(opened['binding']['materials'])}}]
  ip += [{'condition':f"q.block_state('{N}:facing') == {i}",'components':{'minecraft:transformation':{'rotation':[0,-90*i,0]}}}for i in range(4)]
  dump(BP/f'blocks/{short}.json',{'format_version':'1.26.50','minecraft:block':{'description':{'identifier':N+':'+short,'menu_category':{'category':'construction'},'states':{N+':facing':[0,1,2,3],N+':open':[0,1],N+':powered':[0,1]}},'components':ic,'permutations':ip}})
  bindings.append({'kind':'incense','style':style,'item':N+':'+short,'block':N+':'+short,'closed_geometry':closed['geometry']['identifier'],'open_geometry':opened['geometry']['identifier'],'small_particle':speci['small'],'large_particle':speci['large'],'source_large_particle':speci['source_large'],'large_particle_adapter':speci['large_adapter'],'large_y_offset':speci['y_offset'],'large_y_range':speci['y_range'],'source_collision':{'origin':[-3,0,-3],'size':[6,7,6]},'tick_step':3,'damage_period':120,'range':32,'magic_damage':1,'redstone_edge_sync':True,'zombie_villager_conversion':'NOT_ADAPTED_60_TICK','source_push_reaction':'DESTROY','bedrock_movable':'IMMOVABLE_SAFETY_DIVERGENCE','engine_accepted':False})
 ...
 for lc in ['zh_TW','zh_CN','en_US']:
  ...
  for b in bindings:
   if b['kind'] in ['sofa','table','bar_counter','glassware_holder','holder','tilted_rack','circular_rack','bar_cabinet','incense']:continue
 ...
 for page in pages:
  if page['id']==N+':c6_furniture':
   page['title']={'zh_TW':'酒館家具、酒架、掛畫、酒櫃與香薰','zh_CN':'酒馆家具、酒架、挂画、酒柜与香薰','en_US':'Tavern furniture, racks, paintings, cabinets and incense'}
   page['body']={'zh_TW':'家具線已包含高腳凳、沙發、桌子、吧台、彩燈、Glassware Holder、Holder、Pendant Lamps、Tilted/Circular Rack、Batch 14全部14款掛畫，以及Batch 15木質Bar Cabinet與Glass Bar Cabinet、Batch 16全部8款Incense。香薰保留手動OPEN切換與紅石邊沿同步；關閉也持續小粒子，開啟後以低密度server cadence散佈大型粒子，並每120 tick對來源32格膨脹方盒內undead造成1點magic傷害。Java殭屍村民60 tick轉化暫不做，避免用Bedrock不同時序冒充等價。掛畫保留Java牆面／地面／天花板附著與1/16薄碰撞。兩種Bar Cabinet共用Java BarCabinetBlock：每櫃保存left/right兩個精確*_q1..q6瓶子ID；普通瓶可左右各放1瓶，單側佔用時放入／取出會按來源規則自動導向另一／唯一側。來源bar_cabinet_irregular只有brandy與carignan，只能空櫃放入，強制left並single居中，single禁止第二瓶。木櫃與玻璃櫃各自只連同款同朝向櫃體形成single/left/middle/right四態，兩者不互連。Glass Cabinet的Java c:glass_panes展開為無色+16色共17份Bedrock配方。Circular Rack亮度14與低頻End Rod粒子適配仍保留。實機玻璃透明、瓶型位置、掛畫waterlogging、多人與觸控仍待驗收。','zh_CN':'家具线已包含多槽酒架、Batch 14全部14款挂画，以及Batch 15木质Bar Cabinet和Glass Bar Cabinet，并加入Batch 16全部8款Incense。香薰保留手动OPEN切换和红石边沿同步；关闭时也有小粒子，开启后用低密度server cadence生成大粒子，并每120 tick对来源32格范围内undead造成1点magic伤害。Java僵尸村民60 tick转化暂不做。两种酒柜共享Java逻辑：每柜保存left/right两个精确品质瓶ID；brandy/carignan为异形瓶，只能空柜single居中并禁止第二瓶。同款同朝向柜体形成四态连接，木柜与玻璃柜不互连。Glass Cabinet把Java c:glass_panes展开为无色+16色共17份Bedrock配方。挂画与Circular Rack现有适配均保留。实机透明/多人/触控仍待验收。','en_US':'The furniture line now includes the existing racks, all 14 Batch 14 paintings, Batch 15 wooden Bar Cabinet plus Glass Bar Cabinet, and all eight Batch 16 Incense variants. Incense preserves manual OPEN toggling and redstone-edge synchronization; small particles continue while closed, OPEN adds a lower-density server large-particle cadence, and every 120 ticks deals 1 magic damage to undead in the source 32-block-inflated box. Java's exact 60-tick zombie-villager conversion is deliberately not substituted with Bedrock's different vanilla cure timing. Both cabinets use Java BarCabinetBlock semantics: exact left/right *_q1..q6 IDs, Java side redirection when only one regular bottle is present, and source irregular brandy/carignan restricted to an empty cabinet, forced left and rendered centered in single mode with no second bottle. Each cabinet style connects only to the same block type and facing using single/left/middle/right states; wood and glass do not interconnect. Glass Cabinet expands Java c:glass_panes to clear plus all 16 stained panes as 17 Bedrock recipes. Painting attachment and Circular Rack light/particle adaptations remain intact. Real-client glass transparency, bottle placement, painting waterlogging, multiplayer and touch remain NOT_RUN.'}
 ...
 dump(R/'docs/C6-FURNITURE-BINDINGS.json',{'bindings':bindings,'derived_icons':icon_records,'stools':16,'lights':17,'sofas':16,'tables':1,'bar_counters':1,'glassware_holders':1,'holders':1,'pendant_lamps':3,'tilted_racks':1,'circular_racks':1,'paintings':14,'bar_cabinets':2,'incenses':8,'engine_accepted':False})
 ...
 build=load(R/'docs/C5-BUILD.json');build.update({'phase':'C6','version':V,'native_crafting_recipes':108,'effect_hooks':'native + BloodyMary + XPDrain/Zenith/Shriek/UpsideDown/Vision/TombRaider/ArdentHeat/HighHeels adapters','custom_effect_types_pending':coverage['not_implemented'],'furniture':{'stools':16,'lights':17,'sofas':16,'tables':1,'bar_counters':1,'glassware_holders':1,'holders':1,'pendant_lamps':3,'tilted_racks':1,'circular_racks':1,'paintings':14,'bar_cabinets':2,'incenses':8,'new_shaped_recipes':85,'new_shapeless_recipes':13,'source_anchor_y':.875,'source_explicit_rider_offset':-.0625,'native_seat_y':.8125,'sofa_source_anchor_y':.5125,'sofa_native_seat_y':.45,'sofa_connection_states':6,'bar_counter_connection_states':6,'glassware_holder_slots':4,'glassware_holder_light':8,'holder_allowed_bases':14,'holder_display_kinds':15,'holder_redstone':'NOT_ADAPTED','tilted_rack_slots':3,'tilted_rack_allowed_bases':22,'tilted_rack_display_kinds':25,'tilted_rack_redstone':'NOT_ADAPTED','circular_rack_slots':6,'circular_rack_allowed_bases':24,'circular_rack_display_kinds':25,'circular_rack_light':14,'circular_rack_particle':'minecraft:endrod / 20-tick Bedrock cadence','circular_rack_redstone':'NOT_ADAPTED','bar_cabinet_slots':2,'bar_cabinet_irregular_bases':2,'bar_cabinet_connection_states':4,'glass_bar_cabinet_pane_recipes':17,'incense_tick_step':3,'incense_damage_period':120,'incense_range':32,'incense_magic_damage':1,'incense_zombie_villager_conversion':'NOT_ADAPTED_60_TICK','table_axis_states':2,'table_position_states':4,'table_collision_y':[13,16],'light_emission':15},'custom_effects':dict(build['custom_effects'],shriek_attack='native sonicBoom/PvE-only ray adapter',upside_down='Grumm naming adapter over Java 16-block inflated AABB using Bedrock mob-family query',vision='native Glowing radius adapter on Java 50-tick countdown cadence',tomb_raider='30% disarm/drop adapter with 40-tick pickup lock',ardent_heat='per-tick sprint 3x3 source-stone breaking adapter with exhaustion/armor/bare-collision costs',high_heels='grounded blocked-movement one-block auto-step adapter for source +0.5 step-height intent'),'upstream_visual_sync':[{'commit':'c4ec1880bd44cf3139d3ba744ab30bb379cf1416','asset':'string_lights_magenta','fix':'12 rotated zero-thickness planes now have reverse faces; no texture change','engine_acceptance':'NOT_RUN'},{'commit':'b30f34a2e340fed1528954104f93cf2c7e90fd79','asset':'gold_grape_bucket','fix':'official item texture color correction; exact upstream Git blob 7d2452dc5a07f82fd114db6df9e1fb5998878fef / SHA-256 ae8dd1d9802fa02568c3eb457b9e1bacf25d92047faf67bf3dd78bb7ae5691d0','engine_acceptance':'NOT_RUN'},{'commit':'c70eec14b4d8cede23f7274910b8424a8fd49f89','asset':'cocktail_model_only_1','fix':'Brass Heart/Emerald/Godfather/Nether Special: translucent->cutout plus source shade=false faces; automated by sync-plan','engine_acceptance':'NOT_RUN'}],'engine_acceptance':'NOT_RUN'})
 ...
 print('C6 generated: furniture, Holders/Racks, Paintings, wooden+glass Bar Cabinets, eight Incense variants and Pendant Lamps; official post-1.2 visual syncs retained (magenta light + gold grape texture + first c70eec cocktail cutout group).')
