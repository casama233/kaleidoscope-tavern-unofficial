...
| Tomb Raider | `LivingHurtEvent`；指定 entity tag；目標 RNG `<0.3F`；主手耐久改為只剩1後卸下；ItemEntity pickup delay=40 | `entityHurt`＋精確Bedrock ID集合；RNG轉float32；`minecraft:equippable` Mainhand；`minecraft:durability`；`spawnItem`；掉落DP＋`entityItemPickup` before-event封鎖40tick | C6效果持有者限定玩家；Bedrock事件在受傷後，致死一擊與Java時序需實機驗收 |
...
- `core/custom-effects.js`：持續/瞬時自訂效果契約，以及 Upside Down 的AABB相交、Vision半徑／50tick倒數、Tomb Raider目標集合與Java float32機率判定。
- `bedrock/custom-effects.js`：Bloody Mary、XP Drain、Zenith、Shriek、Upside Down、Vision、Tomb Raider 的引擎適配。
...
- `tests/c6-core.test.js`／`tests/c6-runtime.test.js`：C6基線118例，Batch 1～3再增加11例；新11例尚未有此分支Node執行紀錄。
...
- EntityEquippableComponent（主手get/set）：https://learn.microsoft.com/en-us/minecraft/creator/scriptapi/minecraft/server/entityequippablecomponent?view=minecraft-bedrock-stable
- EquipmentSlot（Mainhand）：https://learn.microsoft.com/en-us/minecraft/creator/scriptapi/minecraft/server/equipmentslot?view=minecraft-bedrock-stable
- ItemDurabilityComponent（damage/maxDurability/unbreakable）：https://learn.microsoft.com/en-us/minecraft/creator/scriptapi/minecraft/server/itemdurabilitycomponent?view=minecraft-bedrock-stable
- Dimension.spawnItem：https://learn.microsoft.com/en-us/minecraft/creator/scriptapi/minecraft/server/dimension?view=minecraft-bedrock-stable
- WorldAfterEvents.entityHurt／WorldBeforeEvents.entityItemPickup：https://learn.microsoft.com/en-us/minecraft/creator/scriptapi/minecraft/server/worldafterevents?view=minecraft-bedrock-stable / https://learn.microsoft.com/en-us/minecraft/creator/scriptapi/minecraft/server/worldbeforeevents?view=minecraft-bedrock-stable
