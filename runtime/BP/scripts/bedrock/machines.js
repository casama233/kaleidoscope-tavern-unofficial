...
const CORE=NS+':barrel_core',PART=NS+':barrel_part',TUB=NS+':pressing_tub',TAP=NS+':tap',TAP_OPEN=NS+':open',TUB_FACE='minecraft:block_face',EMPTY_BOTTLE=NS+':empty_bottle',EMPTY_BOTTLE_BLOCK=NS+':bottle_empty',BOTTLE_FACING=NS+':facing',CARDINAL='minecraft:cardinal_direction';
...
function createTapOutput(tap,itemId,carrier){
 ...
 if(carrier?.kind==='block'){
  check(carrier.block===below&&below.typeId===EMPTY_BOTTLE_BLOCK,'TAP_CARRIER_CHANGED');const old=below.permutation,facing=bottleFacingFromCardinal(old.getState(CARDINAL));
  if(parsed){
