"""Offline A4 integrity, source-to-export and reference checks. NOT an engine test.
...
try:
    ...
    for row in registry['models']:
        ...
        if row.get('batch') in ('A2','A3','A4') and row.get('conversion_method') != 'oriented_face_decomposition' and not row.get('face_map'):
    ...
    for key,val in terrain.items():
        check('terrain_ref:'+key, (ROOT/'RP'/(val['textures']+'.png')).exists() and key.startswith(('kt_assets_a1_','kt_assets_a2_','kt_assets_a3_','kt_assets_a4_')))
    identifiers=[]
    ...
    for p in (ROOT/'RP/entity').glob('*.json'):
        ...
        check('entity_refs:'+p.stem, ent['geometry']['default'] in geo_ids and (ROOT/'RP'/(ent['textures']['default']+'.png')).exists())
    ...
    check('lab_manifest_dependency',bp['dependencies']==[{'uuid':rp['header']['uuid'],'version':rp['header']['version']}])
    ...
    for p in (ROOT/'VisualLab_BP/functions').rglob('*.mcfunction'):
        ...
        check('visual_kit_only:'+p.stem,all(x.startswith('give @s ') and x.split()[2] in identifiers for x in commands))
    ...
    check('cookery_not_fabricated',req['required_for_production'] is True and req['bound'] is False)
    ...
    check('expected_batch_totals',len(lock['assets'])==229 and len(pngs)==44 and len(geo_ids)==87 and len(editors)==179 and len(identifiers)==185)
...
report={
    ...
    'source_files':len(lock['assets']),'minecraft_engine':'NOT_RUN','source_jar_comparison':'NOT_RUN',
