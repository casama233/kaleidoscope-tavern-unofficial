...
test('entrypoint registers unique independent item/block custom components and29 recipes',()=>{
 assert.equal(runtimeRegistry().allRecipes().length,29);assert.equal(regs.blocks.size,4);assert.equal(regs.items.size,3);
...
test('explosions exclude runtime machine blocks but retain unrelated blocks',()=>{const p=player(),b=barrel(p),stone=dim.getBlock(target());stone.setType('minecraft:stone');let result;world.beforeEvents.explosion.emit({getImpactedBlocks:()=>[b,stone],setImpactedBlocks:r=>result=r});assert.deepEqual(result,[stone]);});
