...
def snapshot():
 ...
 for name in ['config.json','docs/C1-BUILD.json','docs/C2-BUILD.json','docs/C3-BUILD.json','docs/C4-BUILD.json','docs/C5-BUILD.json','docs/C5-SOURCE-AUDIT.json','docs/C5-EFFECT-COVERAGE.json']:result[name]=hashlib.sha256((ROOT/name).read_bytes()).hexdigest()
 return result
def main():
 ...
 report={'scope':'Byte comparison of two complete C5 runtime/SDK/example build snapshots, not engine validation','files':len(two),'changed_between_builds':sorted(diff),'changed_from_prebuild_snapshot':sorted(initial),'pass':not diff,'sha256':two}
