...
// Java ShakerItem.pourResult emits 20 EFFECT particles; the Bedrock native
// spell emitter uses the same animated spell sprite and needs an explicit tint.
export function cocktailEffect(block,count=1,spread=.1){optional(()=>{
 const variables=new MolangVariableMap();variables.setColorRGBA('variable.color',{red:1,green:1,blue:1,alpha:1});
 for(let i=0;i<count;i++)block.dimension.spawnParticle('minecraft:mobspell_emitter',{
  x:block.location.x+.5+(Math.random()*2-1)*spread,
  y:block.location.y+.5+Math.random()*spread,
  z:block.location.z+.5+(Math.random()*2-1)*spread
 },variables);
});}
