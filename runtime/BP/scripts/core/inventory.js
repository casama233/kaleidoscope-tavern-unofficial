...
export function planInventory(container,selected,take,outputs,makeStack){
 ...
 for(const o of outputs){
  let count=o.count;check(Number.isInteger(count)&&count>0&&count<=1024,'BAD_GIVE');const template=makeStack(o.id,1);
  // Prefer compatible stacks, then the newly-freed held slot, then other empty slots.
  for(let i=0;i<after.length&&count;i++){const slot=after[i];if(slot&&slot.isStackableWith(template)){const n=Math.min(count,slot.maxAmount-slot.amount);slot.amount+=n;count-=n;}}
  const order=[selected,...Array.from({length:after.length},(_,i)=>i).filter(i=>i!==selected)];
  for(const i of order){if(!count)break;if(!after[i]){const n=Math.min(count,template.maxAmount);after[i]=makeStack(o.id,n);count-=n;}}
 ...
 for(let i=0;i<after.length;i++){
  ...
  if((!a!==!b)||(a&&b&&(a.typeId!==b.typeId||a.amount!==b.amount)))changes.push(i);
 }
