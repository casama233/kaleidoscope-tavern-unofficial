...
def main():
    ...
    for name,row in ref['reviewedChanges'].items():
        ...
        if args.baseline:assert sha(args.baseline/name)==row['before'],('Incorrect before hash',name)
    ...
    report={'version':'0.6.43-candidate.1','baseline':ref['baselineCommit'],'javaSourceFilesVerified':verified,
    ...
    (ROOT/'docs/LAUNCH-VALIDATION-0.6.43.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')
